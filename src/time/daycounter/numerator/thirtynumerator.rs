...
fn none_condition (_ymd: YMD, _termination_ymd: YMD) -> bool {
    true
}

fn is_greater_than_thirty (ymd: YMD, _termination_ymd: YMD) -> bool {
    ymd.d > 30
}

fn is_greater_than_or_equal_tothirty (ymd: YMD, _termination_ymd: YMD) -> bool {
    ymd.d >= 30
}
...
fn to_next_month_first(ymd: YMD) -> YMD {
    if ymd.m < 12 {
        YMD {y: ymd.y, m: ymd.m + 1, d: ymd.d}
    } else {
        YMD {y: ymd.y + 1, m: 1, d: ymd.d}
    }
}
...
impl ThirtyNumerator {
    pub fn new(start_date_condition: ThirtyAdjstmentCondition,
               start_date_adjustment: ThirtyAdjustment,
               end_date_condition: ThirtyAdjstmentCondition,
               additional_start_date_condition: ThirtyAdjstmentCondition,
               end_date_adjustment: ThirtyAdjustment,
               schedule: &Schedule) -> ThirtyNumerator {
        ...
        ThirtyNumerator {
            ...
            start_date_adjustment: start_date_adjustment,
            ...
            additional_start_date_condition: additional_start_date_condition,
            end_date_adjustment_impl: get_adjustment_impl(&end_date_adjustment),
            end_date_adjustment: end_date_adjustment,
            termination_ymd: YMD::from_naive_date(schedule.schedule_periods().last().unwrap().calculation_period().end_date())
        }
    }

    pub fn start_date_condition(&self) -> ThirtyAdjstmentCondition {
        self.start_date_condition
    }

    pub fn start_date_adjustment(&self) -> ThirtyAdjustment {
        self.start_date_adjustment
    }

    pub fn end_date_condition(&self) -> ThirtyAdjstmentCondition {
        self.end_date_condition
    }

    pub fn additional_start_date_condition(&self) -> ThirtyAdjstmentCondition {
        self.additional_start_date_condition
    }

    pub fn end_date_adjustment(&self) -> ThirtyAdjustment {
        self.end_date_adjustment
    }

    pub fn termination_date(&self) -> NaiveDate {
        NaiveDate::from_ymd_opt(self.termination_ymd.y, self.termination_ymd.m as u32, self.termination_ymd.d as u32).unwrap()
    }
}
...
impl ThirtyNumeratorGenerator {
    pub fn new(start_date_condition: ThirtyAdjstmentCondition,
               ...
               end_date_condition: ThirtyAdjstmentCondition,
               additional_start_date_condition: ThirtyAdjstmentCondition,
               end_date_adjustment: ThirtyAdjustment) -> ThirtyNumeratorGenerator {
...
impl DayCounterNumeratorGenerator for ThirtyNumeratorGenerator {
    fn generate(&self, schedule_opt: Option<&Schedule>) -> Result<Rc<dyn DayCounterNumerator>, DayCounterGenerationError> {
        if schedule_opt.is_some() {
            let numerator = ThirtyNumerator::new(self.start_date_condition,
                                                                  self.start_date_adjustment,
                                                                  self.end_date_condition,
                                                                  self.additional_start_date_condition,
                                                                  self.end_date_adjustment,
